#!/usr/bin/env python3
"""Run governed P7-T2 Research remediation v6 training."""
...
BASE = _load_module("p7_t2_remediation_pipeline_for_v6", BASE_PIPELINE_PATH)
EVALUATOR = _load_module("p7_t4_research_evaluator_for_v6_training", EVALUATOR_PATH)
ROOT = BASE.ROOT
...
DATASET_RECORD_SCHEMA_VERSION = "6.0.0"
...
def validate_dataset_and_contract_gates(
    ...
    if (
        ...
        or manifest.get("checksum") != artifact_identity(manifest, "checksum")
        or manifest.get("preparedManifestIdentity")
        ...
        raise TrainingPipelineError("dataset manifest: exact approved v6 identity required")
    ...
    if {name: len(items) for name, items in records.items()} != {
        ...
        raise TrainingPipelineError("dataset: exact v6 split counts required")
    ...
    if (
        ...
        raise TrainingPipelineError("training approval: exact active v6 approval required")
    ...
    if (
        ...
        raise TrainingPipelineError("training contract: exact immutable v6 contract required")
    ...
    if (
        ...
        or artifact_identity(evaluator_contract, "artifactIdentity") != EVALUATOR_IDENTITY
