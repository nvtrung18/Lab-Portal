...
FINALIZER_PATH = (
    ROOT / "scripts" / "finalize-p7-t1c-research-remediation-governance-v7.py"
)
REQUEST_IDENTITY = (
    "11946dfb19433e42b55d7d9c3b7e815ad10f0ca45861348003eb0508f62e01cc"
)
...
TRAINING_APPROVAL_IDENTITY = (
    "627f4043fe2c2dcaff21cb0b38d923752d8c928ef8d184b7535f3fe07b2bb3f5"
)
PIPELINE_PATH = ROOT / "scripts" / "training-pipeline-p7-t2-remediation-v7.py"
BACKEND_PATH = ROOT / "scripts" / "p7-t2-real-training-remediation-v7.py"
BUILDER_PATH = ROOT / "scripts" / "build-p7-t2-research-remediation-v7-bundle.py"
VALIDATOR_PATH = ROOT / "scripts" / "validate-p7-t2-research-remediation-v7-bundle.py"
CONFIG_PATH = ROOT / "config" / "p7-t2-training-pipeline-t4-remediation-v7.json"
DATASET_MANIFEST = (
    ROOT / "datasets" / "p7-research-synthetic-training-dataset-v7"
...
class P7T2ResearchRemediationV7TrainingGovernanceTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.finalizer = load_module("p7_t1c_remediation_finalizer_v7", FINALIZER_PATH)

    def test_finalizer_binds_exact_approved_retention_first_request(self):
        ...
        self.assertTrue(approval["scope"]["retainedV6RecordsUnchanged"])
        self.assertEqual(
            ["RESEARCH_UC_004", "RESEARCH_UC_005"],
            approval["scope"]["targetedUseCases"],
        )
        ...
        self.assertEqual(
            ...
            self.finalizer.artifact_identity(manifest),
        )
...
class P7T2ResearchRemediationV7TrainingBundleTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.pipeline = load_module("p7_t2_remediation_pipeline_v7", PIPELINE_PATH)
        cls.backend = load_module("p7_t2_remediation_backend_v7", BACKEND_PATH)
        cls.builder = load_module("p7_t2_remediation_bundle_builder_v7", BUILDER_PATH)
        cls.validator = load_module("p7_t2_remediation_bundle_validator_v7", VALIDATOR_PATH)
        cls.config = json.loads(CONFIG_PATH.read_text(encoding="utf-8"))
        cls.profile = json.loads(PROMPT_PROFILE_PATH.read_text(encoding="utf-8"))

    def test_config_dataset_and_retention_gates_pass(self):
        ...
        self.assertEqual(
            {"train": 96, "validation": 16, "evaluation": 16},
    ...
    def test_backend_accepts_retained_v6_and_targeted_v7_records(self):
        records = [
            json.loads(line)
        ...
        retained = next(record for record in records if record["schemaVersion"] == "6.0.0")
        targeted = next(record for record in records if record["schemaVersion"] == "7.0.0")
        for record in (retained, targeted):
            messages = self.backend.training_messages(record)
            expected = self.profile["assistantProfiles"]["RESEARCH_ASSISTANT"]["systemInstruction"]
    ...
    def test_tokenization_supervises_exactly_one_terminal_eos(self):
        ...
        dataset, metrics = self.backend.tokenize_records_with_eos([record], FakeTokenizer())
