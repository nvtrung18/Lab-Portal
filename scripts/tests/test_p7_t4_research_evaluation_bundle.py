...
class P7T4ResearchEvaluationBundleTests(unittest.TestCase):
    ...
    def test_remediation_v6_bundle_sources_bind_candidate_prompt_and_v2_contracts(self):
        sources = BUILDER.bundle_sources(ROOT, remediation_v6=True)

        self.assertEqual(
            ROOT / "config/p7-t4-research-independent-evaluation-remediation-v6.json",
            sources["config/p7-t4-research-independent-evaluation.json"],
        )
        self.assertEqual(
            ROOT / "evidence/p7-t2-real-training/remediation-v6/adapter-manifest.json",
            sources["evidence/p7-t2-real-training/adapter-manifest.json"],
        )
        self.assertEqual(
        ...
        for reference in (
            ...
            "evidence/p7-t2-real-training/remediation-v6/real-training-evidence.json",
        ):
            self.assertIn(reference, sources)

    def test_remediation_v6_evidence_projects_without_rewriting_training_facts(self):
        reference = (
            "evidence/p7-t2-real-training/remediation-v6/real-training-evidence.json"
        )
        source_payload = (ROOT / reference).read_bytes()
        ...
        projected = BUILDER.build_evaluation_compatibility_evidence(
            source_payload,
            source_reference=reference,
            expected_source_sha256=BUILDER.REMEDIATION_V6_REAL_EVIDENCE_SHA256,
        )
    ...
    def test_remediation_v7_bundle_sources_bind_exact_candidate_and_approval(self):
        sources = BUILDER.bundle_sources(ROOT, remediation_v7=True)

        self.assertEqual(
            ROOT / "config/p7-t4-research-independent-evaluation-remediation-v7.json",
            sources["config/p7-t4-research-independent-evaluation.json"],
        )
        self.assertEqual(
            ROOT / "evidence/p7-t2-real-training/remediation-v7/adapter-manifest.json",
            sources["evidence/p7-t2-real-training/adapter-manifest.json"],
        )
        self.assertEqual(
        ...
        for reference in (
            ...
            "evidence/p7-t2-real-training/remediation-v7/real-training-evidence.json",
        ):
            self.assertIn(reference, sources)

    def test_remediation_v7_evidence_projects_without_rewriting_training_facts(self):
        reference = (
            "evidence/p7-t2-real-training/remediation-v7/real-training-evidence.json"
        )
        source_payload = (ROOT / reference).read_bytes()
        ...
        projected = BUILDER.build_evaluation_compatibility_evidence(
            source_payload,
            source_reference=reference,
            expected_source_sha256=BUILDER.REMEDIATION_V7_REAL_EVIDENCE_SHA256,
        )
    ...
    def test_remediation_v8_bundle_sources_bind_exact_candidate_and_approval(self):
        sources = BUILDER.bundle_sources(ROOT, remediation_v8=True)

        self.assertEqual(
            ROOT / "config/p7-t4-research-independent-evaluation-remediation-v8.json",
            sources["config/p7-t4-research-independent-evaluation.json"],
        )
        self.assertEqual(
            ROOT / "evidence/p7-t2-real-training/remediation-v8/adapter-manifest.json",
            sources["evidence/p7-t2-real-training/adapter-manifest.json"],
        )
        self.assertEqual(
        ...
        for reference in (
            "scripts/research-independent-evaluation-p7-t4-v7.py",
            "config/p7-t4-research-remediation-governance-v8/"
            "external-evaluation-approval-request.json",
            "evidence/p7-t4-research-remediation-v8-external-evaluation-approval.json",
            "evidence/p7-t2-real-training/remediation-v8/real-training-evidence.json",
        ):
            self.assertIn(reference, sources)

    def test_remediation_v8_evidence_projects_without_rewriting_training_facts(self):
        reference = (
            "evidence/p7-t2-real-training/remediation-v8/real-training-evidence.json"
        )
        source_payload = (ROOT / reference).read_bytes()
        ...
        projected = BUILDER.build_evaluation_compatibility_evidence(
            source_payload,
            source_reference=reference,
            expected_source_sha256=BUILDER.REMEDIATION_V8_REAL_EVIDENCE_SHA256,
        )
    ...
    def test_remediation_v10_bundle_sources_bind_exact_candidate_and_approval(self):
        sources = BUILDER.bundle_sources(ROOT, remediation_v10=True)

        self.assertEqual(
            ROOT / "config/p7-t4-research-independent-evaluation-remediation-v10.json",
            sources["config/p7-t4-research-independent-evaluation.json"],
        )
        self.assertEqual(
            ROOT / "evidence/p7-t2-real-training/remediation-v10/adapter-manifest.json",
            sources["evidence/p7-t2-real-training/adapter-manifest.json"],
        )
        self.assertEqual(
        ...
        for reference in (
            "scripts/research-independent-evaluation-p7-t4-v9.py",
            ...
            "evidence/p7-t2-real-training/remediation-v10/real-training-evidence.json",
        ):
            self.assertIn(reference, sources)

    def test_remediation_v10_evidence_projects_without_rewriting_training_facts(self):
        reference = "evidence/p7-t2-real-training/remediation-v10/real-training-evidence.json"
        source_payload = (ROOT / reference).read_bytes()
        source = json.loads(source_payload)
        projected = BUILDER.build_evaluation_compatibility_evidence(
            source_payload,
            source_reference=reference,
            expected_source_sha256=BUILDER.REMEDIATION_V10_REAL_EVIDENCE_SHA256,
        )
        self.assertEqual(source["candidateId"], projected["candidateId"])
        self.assertEqual(source["trainingRunIdentity"], projected["trainingRunIdentity"])
        self.assertEqual(source["actualTraining"], projected["actualTraining"])
    ...
    def test_bundle_sources_reject_multiple_candidate_modes(self):
        ...
        with self.assertRaisesRegex(BUILDER.BundleBuildError, "one candidate mode"):
            BUILDER.bundle_sources(ROOT, remediation_v5=True, remediation_v6=True)
        ...
        with self.assertRaisesRegex(BUILDER.BundleBuildError, "one candidate mode"):
            BUILDER.bundle_sources(ROOT, remediation_v9=True, remediation_v10=True)
