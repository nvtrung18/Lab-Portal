#!/usr/bin/env python3
"""Real single-T4 QLoRA backend for governed remediation v8."""
...
V7_BACKEND_PATH = Path(__file__).with_name("p7-t2-real-training-remediation-v7.py")
DATASET_IDENTITY = "44fb5a05d9a13c4f6e8d39fe7b8fe5a9189dc077b9850da700a17ea76e609b3d"
EXPECTED_TRAIN_RECORDS = 432
EXPECTED_VALIDATION_RECORDS = 72
EXPECTED_CONTRACT_HOLDOUT_RECORDS = 72
OUTPUT_FIELDS = {
    "evalCaseId",
    "observedActionRisk",
    "observedBehavior",
...
V7 = _load_module("p7_t2_remediation_backend_v7_for_v8", V7_BACKEND_PATH)
BASE = V7.BASE
canonical_bytes = V7.canonical_bytes
sha256_bytes = V7.sha256_bytes
PROMPT_PROFILE = V7.PROMPT_PROFILE
SYSTEM_MESSAGE = V7.SYSTEM_MESSAGE
...
def training_messages(record: dict[str, Any]) -> list[dict[str, str]]:
    schema = record.get("schemaVersion") if isinstance(record, dict) else None
    expected_fields = V8_RECORD_FIELDS if schema == "8.0.0" else V7.RECORD_FIELDS
    prompt = record.get("trainingPrompt") if isinstance(record, dict) else None
    target = record.get("trainingTarget") if isinstance(record, dict) else None
    if (
        not isinstance(record, dict)
        or set(record) != expected_fields
        or schema not in {"6.0.0", "7.0.0", "8.0.0"}
        ...
        or (
            schema == "8.0.0"
            and (
                record.get("targetedEvaluationCaseId") != "E-INJECT-001"
                or target.get("structuredOutput") is not None
                or target.get("observedBehavior") != "SAFE_REFUSAL"
            )
        )
    ):
        raise ValueError("training record: exact governed v6/v7/v8 contract required")
    return [
...
def load_training_inputs(manifest_path: Path, config: dict[str, Any]) -> dict[str, Any]:
    if config.get("splits") != {
        ...
        raise ValueError("real remediation v8 requires isolated splits")
    ...
    if (
        ...
        raise ValueError("dataset manifest: exact approved v8 identity required")
    inventory = {
    ...
    for key, filename in (
        ...
        if count != expected_counts[filename]:
            raise ValueError(f"dataset/{filename}: exact v8 record count required")
        records = BASE._load_jsonl(path, count, f"dataset/{filename}")
...
tokenize_records_with_eos = V7.tokenize_records_with_eos
