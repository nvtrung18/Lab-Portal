...
import { useCreateResearchProject, useResearchProjectsByLab } from '../hooks';
...
export function ResearchProjectList({ labId, canCreate }: ResearchProjectListProps) {
  const [isCreateOpen, setIsCreateOpen] = useState(false);
  const { data: projects = [], isError, isLoading, refetch } = useResearchProjectsByLab(labId);
    ...
    <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">Danh sách đề tài nghiên cứu thuộc PTN đang quản lý.</p>
        </div>
        {canCreate ? (
          <button
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          {projects.map((project) => (
            <article key={project.id} className="rounded-md border border-slate-200 p-4">
              ...
              <Link
                className="mt-4 inline-flex rounded-md border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100"
